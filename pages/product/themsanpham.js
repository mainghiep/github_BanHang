app.controller('themsanphamCrtl', function ($http, $scope,$routeParams){
    $scope.sanpham ={
        id: '',
        ten: '',
        anh: '',
    ...
    $scope.oneclickthem = function () {
        console.log('Log thử giá trị biến $scope.product', $scope.sanpham)
        $scope.formStatus = true
        $scope.thongbao = '';
        if ($scope.sanpham.id === '') {
            $scope.formStatus = false;
            $scope.thongbao = 'ID Không Được Bỏ Trống';
            return;
        }
